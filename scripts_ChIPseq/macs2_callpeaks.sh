...
macs2 callpeak -t mapped/chip/downsample/${x}.dupmark.sorted.bam \
  ...
  --nomodel -g 373128865 \
  --outdir ${macs2_out} -n ${x}
                                                 
                                                 
awk -F"\t" -v q=${q} 'BEGIN{OFS="\t"} $9>=q {print}' ${macs2_out}/${x}_peaks.narrowPeak > ${macs2_out}/noMask_qval${q}/${x}_peaks.narrowPeak                                             
    
bedtools intersect -v -wa \
  -a ${macs2_out}/noMask_qval${q}/${x}_peaks.narrowPeak \
  -b ${gs_regions} > ${macs2_out}/gsMask_qval${q}/${x}_peaks.narrowPeak                                             
